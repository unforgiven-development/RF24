...
/* for Arduino mode, where "RF24_SPI_TRANSACTIONS" is not defined */
#elif defined(ARDUINO) && !defined (RF24_SPI_TRANSACTIONS)
	/*
	 * The (minimum) ideal SPI bus speed is 2x the desired data rate:
	 * If we assume 2mbps data rate and 16MHz clock, a divider of 4 is the minimum we want.
	 *
	 * A divider of 4 results in the following CLK (system clock) to BUS (SPI BUS clock frequency) relationships:
	 *     CLK:BUS
	 * -  8MHz:2MHz
	 * - 16MHz:4MHz
	 * - 20MHz:5MHz
	 *
	 * Below, as long as we're not using software-based SPI, we'll setup the SPI bus to use MODE 0, MSB-first ordering,
	 * and a clock divider of 2 (resulting in an SPI frequency of 8MHz for most setups):
	 */
#if !defined (SOFTSPI)
...
/* for RF24 on Raspberry Pi */
	...
	/* for all non-Linux applications */
...
void RF24::print_status(uint8_t status) {
	printf_P(PSTR("STATUS\t\t = 0x%02x RX_DR=%x TX_DS=%x MAX_RT=%x RX_P_NO=%x TX_FULL=%x\r\n"), status, (status & _BV(RX_DR)) ? 1 : 0, (status & _BV(TX_DS)) ? 1 : 0, (status & _BV(MAX_RT)) ? 1 : 0, ((status >> RX_P_NO) & 0x07), (status & _BV(TX_FULL)) ? 1 : 0);
}
...
RF24::RF24(uint16_t _cepin, uint16_t _cspin) : ce_pin(_cepin), csn_pin(_cspin), p_variant(false), payload_size(32), dynamic_payloads_enabled(false), addr_width(5), csDelay(5)//,pipe0_reading_address(0)
{
...
static const char rf24_datarate_e_str_0[]
PROGMEM = "1MBPS";
static const char rf24_datarate_e_str_1[]
PROGMEM = "2MBPS";
static const char rf24_datarate_e_str_2[]
PROGMEM = "250KBPS";
...
static const char rf24_model_e_str_0[]
PROGMEM = "nRF24L01";
static const char rf24_model_e_str_1[]
PROGMEM = "nRF24L01+";
...
static const char rf24_crclength_e_str_0[]
PROGMEM = "Disabled";
static const char rf24_crclength_e_str_1[]
PROGMEM = "8 bits";
static const char rf24_crclength_e_str_2[]
PROGMEM = "16 bits";
...
static const char rf24_pa_dbm_e_str_0[]
PROGMEM = "PA_MIN";
static const char rf24_pa_dbm_e_str_1[]
PROGMEM = "PA_LOW";
static const char rf24_pa_dbm_e_str_2[]
PROGMEM = "PA_HIGH";
static const char rf24_pa_dbm_e_str_3[]
PROGMEM = "PA_MAX";
