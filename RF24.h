...
public:
	 ...
	 *
	 * Creates a new instance of this driver. Before using, you create an instance and send in the unique pins that this
	 * chip is connected to.
	 *
	 * \param[in]	_cepin	The pin attached to Chip Enable on the RF module
	 * \param[in]	_cspin	The pin attached to Chip Select
	 */
	...
	/**
	 * \brief Optional Linux Constructor
	 *
	 * Creates a new instance of this driver. Before using, you create an instance and send in the unique pins that this
	 * chip is connected to.
	 *
	 * \param[in]	_cepin		The pin attached to Chip Enable on the RF module
	 * \param[in]	_cspin		The pin attached to Chip Select
	 * \param[in]	spispeed	For RPi, the SPI speed in MHZ ie: BCM2835_SPI_SPEED_8MHZ
	*/
	...
	/**
	 * \brief Read the available payload
	 *
	 * The size of data read is the fixed payload size, see getPayloadSize()
	 *
	 * \note I specifically chose 'void*' as a data type to make it easier for beginners to use, with No casting needed.
	 *
	 * \note No longer boolean. Use available() to determine if packets are available. Interrupt flags are now cleared
	 * during reads instead of when calling the available() function.
	 *
	 ...
	 * \endcode
	 *
	 * \return No return value. Use available() instead.
	 */
...
s   *
	...
	/**
	 * \fn bool failureDetected
	 *
	 * Enable error detection by un-commenting
	 * \code
	 * #define FAILURE_HANDLING
	 * \endcode in RF24_config.h\n
	 *
	 * If a failure has been detected, it usually indicates a hardware issue. By default the library will cease
	 * operation when a failure is detected. This should allow advanced users to detect and resolve any intermittent
	 * hardware issues.
	 *
	 * In most cases, the radio must be re-enabled via radio.begin(); and the appropriate settings applied after a
	 * failure occurs, if wanting to re-enable the device immediately.\n
	 *
	 * \b USAGE:
	 * (Failure handling must be enabled per above)
	 * \code
	...
	/**
	 * \name Optional Configurators
	 * Methods you can use to get or set the configuration of the chip. None are required. Calling begin() sets up a
	 ...
	 *
	 * \param[in]	delay	How long to wait between each retry, in multiples of 250us. Max is 15; range: 250us - 4000us
	 * \param[in]	count	How many retries before giving up. Max 15.
	 */
	void setRetries(uint8_t delay, uint8_t count);

	/**
	 * Set RF communication channel, from a range of 0 - 125.
	 *
	 * \brief Set RF channel
	 *
	 * \param[in]	channel	Which RF channel to communicate on. Valid range: 0 - 125
	 */
